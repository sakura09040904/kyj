...
# 메서드 영역
    ...
    def dataload(self):
        ...
        self.rs_total.append(rs)
        self.nowstr_total.append(nowstr)

        if self.count > 20 :
            self.count = 20
            self.rs_total = self.rs_total[1:20]
            self.nowstr_total = self.nowstr_total[1:20]
        i=0    
        for i in range(self.count):
            self.tbl.setItem(i, 0, QTableWidgetItem(self.nowstr_total[i]))
            j = 0
            for j in range(10):
                self.tbl.setItem(i, j+1, QTableWidgetItem(str(self.rs_total[i][j])))
